...
user_type_ind_col = tf.feature_column.indicator_column(user_type_col)
...
item_type_col = tf.feature_column.categorical_column_with_vocabulary_list(key="productType1",
                                                                           vocabulary_list=type_vocab)
item_type_ind_col = tf.feature_column.indicator_column(item_type_col)
item_type_emb_col = tf.feature_column.embedding_column(item_type_col, 10)

# fm first-order categorical items
cat_columns = [product_ind_col, user_ind_col, user_type_ind_col, item_type_ind_col]
...
second_order_cat_columns_emb = [tf.keras.layers.DenseFeatures([item_type_emb_col])(inputs),
                                tf.keras.layers.DenseFeatures([product_emb_col])(inputs),
                                tf.keras.layers.DenseFeatures([user_type_emb_col])(inputs),
