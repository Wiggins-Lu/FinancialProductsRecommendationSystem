...
class UdfFunction:
    ...
    def sortF(product_list, timestamp_list):
        ...
        """
        pairs = []
        for m, t in zip(product_list, timestamp_list):
            pairs.append((m, t))
        # sort by time
...
def processItemSequence(spark, rawSampleDataPath):
    ...
    # userSeq.select("userId", "productIdStr").show(10, truncate = False)
...
def embeddingLSH(spark, productEmbMap):
    ...
    for key, embedding_list in productEmbMap.items():
        ...
        productEmbSeq.append((key, Vectors.dense(embedding_list)))
    ...
    bucketModel.approxNearestNeighbors(productEmbDF, sampleEmb, 5).show(truncate=False)
...
def trainItem2vec(spark, samples, embLength, embOutputPath, saveToRedis, redisKeyPrefix):
    ...
    with open(embOutputPath, 'w') as f:
        for product_id in model.getVectors():
            vectors = " ".join([str(emb) for emb in model.getVectors()[product_id]])
            f.write(product_id + ":" + vectors + "\n")
